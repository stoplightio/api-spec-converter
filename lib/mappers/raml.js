...
RAML.prototype.mapResponse = function(response) {
  var data = []
  for(var code in response) {
    if (!response[code].body) {
      continue
    }
    var result = this.mapBody(response[code].body)
    result.codes = [code]
...
RAML.prototype.createEndpoint = function(me, resource, baseURI) {
  ...
  for (var i = 0; i < resource.methods.length; i++) {
    ...
    if (method.responses) {
      endpoint.Responses = this.mapResponse(method.responses)
    }
...
RAML.prototype.loadFile = function (filePath) {
  var me = this
  return function(cb) {
    raml.loadFile(filePath).then(function(data) {
      me.data = data
      cb()
    }, function(error) {
