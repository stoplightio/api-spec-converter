...
Swagger.prototype._constructSwaggerMethod = function(endpoint, parameters, responses, env) {
  ...
  if (_.isArray(consumes) && endpoint.Consumes && !_.isEqual(env.Consumes, consumes)) {
    resultSwaggerMethod.consumes = consumes.filter(_.isString);
  }

  if (_.isArray(produces) && endpoint.Produces && !_.isEqual(env.Produces, produces)) {
...
Swagger.prototype._mapSchema = function(slSchemas) {
  ...
  //     var definition = convertRefAttFromObject(jsonHelper.parse(schema.Definition));
...
Swagger.prototype._export = function () {
  ...
  var parsedHost = url.parse(env.Host);

  // If host has path on it, prepend to base path
  if (parsedHost.path) {
    swaggerDef.BasePath = urlHelper.join(parsedHost.path, env.BasePath);
  }
