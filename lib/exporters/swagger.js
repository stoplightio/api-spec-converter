...
Swagger.prototype._getResponseTypes = function(endpoint, defaultResponseType) {
  var result = [],
      defRespT = defaultResponseType || [];

  result = endpoint.Produces.concat(defRespT);
  defRespT.forEach(function(mimeType) {
    result = _.without(result, mimeType);
  });
...
Swagger.prototype._getRequestTypes = function(endpoint, parameters, defaultRequestType) {
  ...
  for(var i in parameters) {
    if (parameters[i].type && parameters[i].type === 'file') {
      ...
      if (endpoint.Consumes) {
        endpoint.Consumes.forEach(function(mimeType) {
          if (typesToInclude.indexOf(mimeType) >= 0) {
            result.push(mimeType);
          }
        });
      }

      defReqT.forEach(function(mimeType) {
        if(typesToInclude.indexOf(mimeType) >= 0) {
          result.push(mimeType);
        }
      });
  ...
  result = result.concat(endpoint.Consumes || []);
  defReqT.forEach(function(mimeType) {
    result = _.without(result, mimeType);
  });
...
function mapResponseBody(res, produces) {
...
Swagger.prototype._mapResponseBody = function(endpoint) {
  ...
  for(var i in slResponses) {
    var res = slResponses[i],
        item = mapResponseBody(res, endpoint.Produces);
...
Swagger.prototype._mapEndpoints = function (swaggerDef, env) {
  ...
  for(var i in endpoints) {
    ...
    var responses = _.assign({}, this._mapEndpointTraitResponses(endpoint), this._mapResponseBody(endpoint));

    if (_.isEmpty(this._getResponseTypes(endpoint))) {
