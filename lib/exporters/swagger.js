...
Swagger.prototype.getResponseTypes = function(slResponses) {
  var result = [];
...
Swagger.prototype.getRequestTypes = function(endpoint, parameters, defaultRequestType) {
  ...
  for(var i in parameters) {
    if (parameters[i].type && parameters[i].type === 'file') {
      ...
      if (typesToInclude.indexOf(endpoint.Body.mimeType) >= 0) {
        result.push(endpoint.Body.mimeType);
...
Swagger.prototype.validateParameters = function(parameters) {
...
Swagger.prototype.constructSwaggerMethod = function(endpoint, parameters, responses, env) {
  var resultSwaggerMethod = {
    description: endpoint.Description,
    operationId: endpoint.Name,
    consumes: this.getRequestTypes(endpoint, parameters, env.DefaultRequestType),
    produces: this.getResponseTypes(endpoint.Responses),
  ...
  for(var i in endpoints) {
    ...
    if (!swaggerDef.paths[endpoint.Path]) {
      swaggerDef.paths[endpoint.Path] = {
         parameters: this.validateParameters(this._mapURIParams(endpoint.PathParams))
      };
    }
    ...
    swaggerDef.paths[endpoint.Path][endpoint.Method] = this.constructSwaggerMethod(endpoint, parameters, responses, env);
