...
Swagger.prototype._getResponseTypes = function(endpoint, defaultResponseType) {
  var result = [];
  endpoint.Produces.forEach(function(mimeType) {
    defaultResponseContentType.forEach(function(defMimeType) {
      if (result.indexOf(mimeType) === -1 && mimeType !== defMimeType) {
...
Swagger.prototype._getRequestTypes = function(endpoint, parameters, defaultRequestType) {
  ...
  for(var i in parameters) {
    if (parameters[i].type && parameters[i].type === 'file') {
      ...
      ep.Consumes.forEach(function(mimeType) {
        if (typesToInclude.indexOf(mimeType) >= 0) {
          result.push(mimeType);
        }
      });

      defaultRequestType.forEach(function(mimeType) {
        if(typesToInclude.indexOf(mimeType) >= 0) {
          result.push(mimeType);
        }
      });
