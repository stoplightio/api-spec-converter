    ...
    for (var property in body.properties) {
      ...
      switch(slRequestBody.mimeType) {
        case 'application/json':
          ...
          break;
        default:
          param.type = body.properties[property].type || 'string';
          param.name = property;
          param.in = 'formData';
      }
      ...
      if (body.properties[property].hasOwnProperty('description')) {
        ...
        //description is saved inside schema in stoplight format, remove it from there
        delete body.properties[property].description;
      }

      result.push(param);
    }
  } else if (body.type === 'array' || body.hasOwnProperty('$ref')) {
    //Handle array or root reference
    var param = body;
    result.push(param);
  } else {
    //TODO any other case?
