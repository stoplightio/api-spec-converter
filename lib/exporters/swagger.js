    ...
    switch(type) {
      ...
      case 'oauth2':
        ...
        for (var i in slScopes) {
          ...
          var scopeKey = Object.keys(scope)[0];
          swaggerScopes[Object.keys(scope)[0]] = scope[scopeKey];
        }
...
Swagger.prototype._mapRequestBody = function(slRequestBody) {
  ...
  for (var property in body.properties) {
    ...
    if (body.properties[property].type === 'object' ||
        body.properties[property].hasOwnProperty('$ref')) {
    ...
    if (body.properties[property].hasOwnProperty('$ref')) {
      param.schema = {
        '$ref': body.properties[property]['$ref']
      };
    }
...
Swagger.prototype._export = function () {
  ...
  for(var i in endpoints) {
    ...
    swaggerDef.securityDefinitions = this._mapSecurityDefinitions(this.project.securitySchemes);
    ...
    if (endpoint.SecuredBy && !endpoint.SecuredBy.none) {
      swaggerDef.paths[endpoint.Path][endpoint.Method]['security'] = this._mapEndpointSecurity(endpoint.SecuredBy, this.project.securitySchemes);
    }
  }
