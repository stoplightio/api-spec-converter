...
function _mapEndpoint(self, endpoint) {
...
StopLightX.prototype._mapEndpoints = function() {
  var self = this;
  self.project.Endpoints.forEach(endpoint => _mapEndpoint(self, endpoint));
};

StopLightX.prototype._mapSchemas = function() {
  var self = this;

  self.project.Schemas.forEach((schema) => {
    var obj = {
      id: schema.Id,
      name: schema.Name,
    };
...
StopLightX.prototype._mapTests = function(tests, namespace) {
  return tests.reduce((res, test) => {
    ...
    exportTest.steps = test.steps.map((step) => {
      if (step.test) {
        return {
...
StopLightX.prototype._export = function() {
  ...
  this.data[prefix] = {
    ...
    version: env.toJSON(),
    functions: this.project.UtilityFunctions.reduce((res, item) => {
      res[item.name] = item.toJSON();
      return res;
    }, {}),
    textSections: this.project.Texts.reduce((res, item) => {
