...
RAML08.prototype.mapRequestBodyForm = function(bodyData) {
  ...
  if (bodyData.required && bodyData.required.length > 0) {
    for (i in bodyData.required) {
      if (!{}.hasOwnProperty.call(bodyData.required, i)) continue;
      requiredParam = bodyData.required[i];
...
RAML08.prototype.mapSecuritySchemes = function(securitySchemes) {
  return _.map(securitySchemes, (v, k) => {
    var m = {};
    m[k] = v;
    return m;
  });
...
RAML08.prototype.setMethodDisplayName = function() {};
