...
Exporter.prototype.export = function (format, cb) {
  try {
    this._export();
    var formattedData, exportedData = this._getData(format);
    ...
    importer.loadData(formattedData, function(err){
      if (err) {
        return cb(err, exportedData);
      }
      try {
        importer.import();
