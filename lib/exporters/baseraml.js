...
RAML.prototype._mapSecurityScheme = function(slSecuritySchemes) {
  ...
  if ({}.hasOwnProperty.call(slSecuritySchemes, 'oauth2')) {
    var oauth2Name = slSecuritySchemes.oauth2.name || 'oauth2';
  ...
  if ({}.hasOwnProperty.call(slSecuritySchemes, 'apiKey')) {
    ...
    if (!_.isEmpty(slSecuritySchemes.apiKey.queryString)) {
      ...
      for (var apiKeyIndex in slSecuritySchemes.apiKey.queryString) {
				if (!{}.hasOwnProperty.call(slSecuritySchemes.apiKey.queryString, apiKeyIndex)) continue;
        var value = slSecuritySchemes.apiKey.queryString[apiKeyIndex];
...
RAML.prototype._export = function() {
  ...
  for (var i in endpoints) {
    ...
    if (endpoint.Method.toLowerCase() === 'post' ||
      ...
      var mimeTypeConsumes = getDefaultMimeType(endpoint.Consumes, ramlDef.mediaType);
      method.body = this._mapRequestBody(endpoint.Body, mimeTypeConsumes);
    }

    method.headers = this._mapNamedParams(endpoint.Headers);

    var mimeTypeProduces = getDefaultMimeType(endpoint.Produces, ramlDef.mediaType);
