...
function RAMLDefinition(title, env) {
  ...
  this.mediaType = (env.Produces && env.Produces.length > 0) ? env.Produces[0] : '';
...
RAML.prototype._export = function () {
  ...
  for(var i in endpoints) {
    ...
    if (endpoint.Method.toLowerCase() === 'post' ||
      ...
      var mimeType = (endpoint.Consumes && endpoint.Consumes.length > 0) ? endpoint.Consumes[0]:null;
      method.body = this._mapRequestBody(endpoint.Body, mimeType);
    }

    method.headers = this._mapNamedParams(endpoint.Headers);

    // TODO: Don't know if this is the best way to do this.
    var mimeType = (endpoint.Produces && endpoint.Produces.length > 0) ? endpoint.Produces[0]:null;
