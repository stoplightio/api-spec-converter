  ...
  if (bodyData.body) {
    ...
    switch(mimeType) {
      ...
      case 'application/x-www-form-urlencoded':
         ...
         */
        for (var i in body[mimeType].properties) {
          var property = body[mimeType].properties[i];
        ...
        if (parsedBody.required && parsedBody.required.length > 0) {
          for(var i in parsedBody.required) {
            ...
            if (body[mimeType]['properties'][requiredParam]) {
              body[mimeType]['properties'][requiredParam].required = true;
            }
          }
        }
        break;
    ...
    if(resBody.codes && Array.isArray(resBody.codes) && resBody.codes.length>0) {
      ...
      if (resBody.mimeType) {
        responses[code]['body'][type] = {
          type: jsonHelper.format(resBody.body),
...
RAML.prototype._mapSchema = function(slSchemas) {
  ...
  for (var i in slSchemas) {
    ...
    var definition = convertRefAttFromObject(schema.Definition);
...
RAML.prototype._export = function () {
  ...
  ramlDef.types = this._mapSchema(this.project.Schemas);
