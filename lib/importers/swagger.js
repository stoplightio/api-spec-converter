...
Swagger.prototype._mapSecurityDefinitions = function(securityDefinitions) {
  ...
  var key;
  var scope;
  for (name in securityDefinitions) {
    if (!{}.hasOwnProperty.call(securityDefinitions, name)) continue;
    type = securityDefinitions[name].type;
    if (!{}.hasOwnProperty.call(result, type)) {
      result[type] = {};
    }
    sd = securityDefinitions[name];
    switch (type) {
      case 'apiKey':
        keyPlaceHolder = (sd.in === 'header') ? 'headers' : 'queryString';
        if (!{}.hasOwnProperty.call(result[type], keyPlaceHolder)) {
      ...
      case 'oauth2':
        ...
        slScopes = [];
        swaggerScopes = sd.scopes;
        if (swaggerScopes) {
          for (key in swaggerScopes) {
            if (!{}.hasOwnProperty.call(swaggerScopes, key)) continue;
            scope = {};
            scope.name = key;
...
Swagger.prototype._mapSchema = function(schemaDefinitions) {
  ...
  var re;
  var prop;
  for (schemaName in schemaDefinitions) {
    if (!{}.hasOwnProperty.call(schemaDefinitions, schemaName)) continue;
    sd = new Schema(schemaName);
    sd.Name = schemaName;
    // create a clone to remove extension properties
    schemaDataClone = _.clone(schemaDefinitions[schemaName]);
    re = /^x-/; // properties to avoid
    for (prop in schemaDataClone) {
      if ({}.hasOwnProperty.call(schemaDataClone, prop) && prop.match(re)) {
...
Swagger.prototype._mapQueryString = function(params, skipParameterRefs) {
  ...
  var param;
  for (i in params) {
    if (!{}.hasOwnProperty.call(params, i)) continue;
    param = params[i];
...
Swagger.prototype._mapResponseBody = function(responseBody, skipParameterRefs, resolvedResponses, $refs) {
  ...
  var bodySchema;
  var examples;
  for (code in responseBody) {
    if (!{}.hasOwnProperty.call(responseBody, code)) continue;
    res = {body: {}, example: '', codes: []};
    description = '';
    ...
    if (needDeReferenced(responseBody[code]) && resolvedResponses) {
      ...
      bodySchema = responseBody[code].schema;
      if (needDeReferenced(responseBody[code].schema)) {
        description = resolvedResponses[code].description || '';
        bodySchema = resolvedResponses[code].schema;
      }
      res.body = bodySchema;
    }

    if ({}.hasOwnProperty.call(responseBody[code], 'examples') && _.isEmpty(responseBody[code].examples)) {
      examples = responseBody[code].examples;
...
Swagger.prototype._parseData = function(dataOrPath, cb, options) {
  ...
  dataCopy = _.cloneDeep(dataOrPath);
  parser.validate(dataCopy, validateOptions)
  .then(() => me._doParseData(dataOrPath, cb, options || {}))
  .catch(cb);
};
...
Swagger.prototype._doParseData = function(dataOrPath, cb, options) {
  ...
  // without validation
  parser.parse(dataOrPath, options, (err, api) => {
    var parseFn;
    if (err) {
      cb(err);
    } else {
...
Swagger.prototype.loadData = function(data, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    self._parseData(parsedData, (err) => {
      if (err) {
        return reject(err);
      }
...
Swagger.prototype.findDefaultMimeType = function(mimeTypes) {
  ...
  for (i in mimeTypes) {
    if (!{}.hasOwnProperty.call(mimeTypes, i)) continue;
...
Swagger.prototype._mapEndpoints = function(consumes, produces) {
  ...
  var securityName;
  var scheme;
  for (path in this.data.paths) {
    if (!{}.hasOwnProperty.call(this.data.paths, path)) continue;
    methods = this.data.paths[path];
    pathParams = {};
    ...
    for (method in methods) {
      if (!{}.hasOwnProperty.call(methods, method)) continue;
      currentMethod = methods[method];
      currentMethodResolved = this.dereferencedAPI ? this.dereferencedAPI.paths[path][method] : currentMethod;

      if (method === 'parameters') {
        continue;
      }
      endpoint = new Endpoint(currentMethod.summary || '');
      ...
      params = currentMethod.parameters;
      c = [];
      ...
      if (consumes && _.isArray(consumes) && c.length) {
        for (index1 in consumes) {
          c = _.without(c, consumes[index1]);
        }
      }
      ...
      if (currentMethod.consumes && _.isArray(currentMethod.consumes)) {
        ...
        if (consumes && _.isArray(consumes) && c.length) {
          for (index2 in consumes) {
            c = _.without(c, consumes[index2]);
          }
        }
      ...
      if (endpoint.Method.toLowerCase() !== 'get' &&
        ...
        body = this._mapRequestBody(currentMethod.parameters, currentMethodResolved.parameters);
      ...
      if (currentMethod.produces && _.isArray(currentMethod.produces)) {
        p = _.uniq((endpoint.Produces && _.isArray(endpoint.Produces)) ? endpoint.Produces.concat(currentMethod.produces) : currentMethod.produces);
        if (produces && _.isArray(produces) && p.length) {
          for (index3 in produces) {
            p = _.without(p, produces[index3]);
          }
        }
      ...
      responses = this._mapResponseBody(currentMethod.responses, true, currentMethodResolved.responses, this.$refs);
      ...
      if (currentMethod.security) {
        securities = currentMethod.security;
        for (securityIndex in securities) {
          if (!{}.hasOwnProperty.call(securities, securityIndex)) continue;
          keys = Object.keys(securities[securityIndex]);
          securityName = keys[0];
          scheme = _.get(this, ['data', 'securityDefinitions', securityName]);
          if (!scheme) {
            // definition error
            continue;
          }
...
Swagger.prototype._mapTraits = function(parameters, responses) {
  ...
  var trait;
  var headerParamsKey;
  var headerParamsTrait;
  var traitResponsesKey;
  var traitResponsesTrait;

  for (k in parameters) {
    if (!{}.hasOwnProperty.call(parameters, k)) continue;
    param = parameters[k];
    parts = k.split(':');
    name = k;
  ...
  for (r in responses) {
    if (!{}.hasOwnProperty.call(responses, r)) continue;

    response = responses[r];
    resName = r;
    resCode = 200;
    resNameParts = r.split(':');
  ...
  for (queryParamsKey in queryParams) {
    if (!{}.hasOwnProperty.call(queryParams, queryParamsKey)) continue;
    trait = traits[queryParamsKey] || {
      _id: queryParamsKey,
      name: queryParamsKey,
      request: {},
      responses: [],
    ...
    trait.request.queryString = this._mapQueryString(queryParams[queryParamsKey]);
    traits[queryParamsKey] = trait;
  }

  for (headerParamsKey in headerParams) {
    if (!{}.hasOwnProperty.call(headerParams, headerParamsKey)) continue;
      headerParamsTrait = traits[headerParamsKey] || {
      _id: headerParamsKey,
      name: headerParamsKey,
      request: {},
      responses: [],
    };

    headerParamsTrait.request.headers = this._mapRequestHeaders(headerParams[headerParamsKey]);
    traits[headerParamsKey] = headerParamsTrait;
  }

  for (traitResponsesKey in traitResponses) {
    if (!{}.hasOwnProperty.call(traitResponses, traitResponsesKey)) continue;
    traitResponsesTrait = traits[traitResponsesKey] || {
      _id: traitResponsesKey,
      name: traitResponsesKey,
      request: {},
      responses: [],
    };
    traitResponsesTrait.responses = this._mapResponseBody(traitResponses[traitResponsesKey]);
    traits[traitResponsesKey] = traitResponsesTrait;
  }
...
Swagger.prototype._import = function() {
  ...
  schemas = this._mapSchema(this.data.definitions);
  for (i in schemas) {
    if (!{}.hasOwnProperty.call(schemas, i)) continue;
