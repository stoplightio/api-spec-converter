...
Swagger.prototype._mapSchema = function(schemaDefinitions) {
  ...
  for (var schemaName in schemaDefinitions) {
    ...
    sd.Definition = jsonHelper.cleanSchema(schemaDataClone);
    result.push(sd);
  }
  return result;
};
...
Swagger.prototype._mapResponseBody = function(responseBody, skipParameterRefs, resolvedResponses) {
  var data = [];
  for (var code in responseBody) {
    ...
    res.body = jsonHelper.cleanSchema(res.body);
  ...
  for (var path in this.data.paths) {
    ...
    for (var method in methods) {
      ...
      if (currentMethod.consumes && _.isArray(currentMethod.consumes)) {
        endpoint.Consumes = currentMethod.consumes;
        // endpoint.Consumes = _.uniq(currentMethod.consumes ? currentMethod.consumes.concat(consumes):consumes||[]);
      } else {
      ...
      //   }
      // }
      if (currentMethod.consumes && currentMethod.consumes.length) {

      }
      endpoint.Produces = _.uniq(currentMethod.produces ? currentMethod.produces.concat(produces):produces);
      var responses = this._mapResponseBody(currentMethod.responses, true, currentMethodResolved.responses);
