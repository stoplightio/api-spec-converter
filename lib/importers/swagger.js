  ...
  for (var i in params) {
    ...
    if (param['$ref']) {
      ...
      if (parts[0] === 'parameters') {
        param = _.get(this.data, parts.join('.'));
        if (!param) {
          param = params[i];
        }
      }
...
Swagger.prototype._mapResponseBody = function(responseBody, resType) {
  ...
  for (var code in responseBody) {
    ...
    if (responseBody[code].schema) {
      ...
      if (responseBody[code].schema['$ref']) {
        // TODO: Once we support headers, then support headers from swagger spec in responses.
        var parts = responseBody[code].schema['$ref'].split('/');
        if (parts[0] === '#') {
          parts.shift();
        }
        if (parts[0] === 'responses') {
          var respKey = this.data[parts.join('.')];
          var resp = param = _.get(this.data, respKey);
          schema = {};
          if (resp && resp.schema) {
            schema = resp.schema;
          }
...
Swagger.prototype.loadData = function(data, cb) {
  ...
  parser.parse(parsedData, function(err, api){
    if (err) {
      cb(err);
    } else {
      me.data = api;
