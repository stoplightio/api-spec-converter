...
RAML08.prototype.mapRequestBody = function(methodBody) {
  ...
  for (var i in methodBody) {
    ...
    if (mimeType.schema && mimeType.schema()) {
      var body = jsonHelper.parse(YAML.load(mimeType.dump()));
      if (body && body[mimeType.name()]) {
        data.body = body[mimeType.name()].schema;
      }
    }
  }
...
RAML08.prototype.mapSchema = function(schemData) {
    ...
    for (var i in schemData) {
        ...
        var def = RAML08.prototype.convertRefToModel(jsonHelper.parse(YAML.load(schema.dump())));
        if (def && def[schemaName]) {
          sd.Definition = def[schemaName];
        }
