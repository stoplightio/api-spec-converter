...
RAML.prototype._mapEndpoint = function(resource, baseURI, pathParams) {
  ...
  for (var i in resource.methods) {
    ...
    if (method.body) {
      endpoint.Consumes = mapMimeTypes(method.body);
    ...
    if (method.responses) {
      ...
      for(var code in method.responses) {
        ...
        produces = produces.concat(mapMimeTypes(method.responses[code].body));
      }
      endpoint.Produces = _.uniq(produces);
