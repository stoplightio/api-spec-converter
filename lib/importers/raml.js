...
RAML.prototype._mapSchema = function(schemData) {
  ...
  for (var i in schemData) {
    ...
    var sd = new Schema(schema.name());
    sd.Name = schema.name();

    var definition;
    if (jsonHelper.isEmptySchema(schema)) {
      definition = YAML.load(schema.dump())[schema.name()];
    } else {
    ...
    sd.Definition = convertRefAttFromObject(definition);
...
function convertRefAttFromObject(object) {
  for (var id in object) {
    if (object.hasOwnProperty(id)) {
      ...
      if (id == 'type' && typeof val === 'string' && ramlHelper.getScalarTypes.indexOf(val) < 0) {
        ...
        object[id] = convertRefAttFromObject(val);
      }
    }
  }
...
RAML.prototype._import = function() {
  ...
  if (this.data.types) {
    var schemas = this._mapSchema(this.data.types());
