...
RAML.prototype._mapRequestBody = function (methodBody) {
  ...
  for (var i in methodBody) {
    ...
    if (mimeType.properties) {
      ...
      data.body = getBodyFrom(mimeType.schema());
    }
    else if (mimeType.type && Array.isArray(mimeType.type()) && mimeType.type().length > 0) {
      data.body = getBodyFrom(mimeType.type());
    }
  }
...
function getBodyFrom(objects) {
  var first = objects[0];
  return jsonHelper.parse(first ? first : '');
}
...
RAML.prototype._mapSchema = function(schemData) {
  ...
  for (var i in schemData) {
    ...
    sd.Name = schema.name();
    sd.Definition = JSON.parse(schema.type());
    schemas.push(sd);
  }
  return schemas;
};
