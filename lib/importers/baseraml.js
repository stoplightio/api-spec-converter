...
// >>>>>>> master:lib/importers/baseraml.js
};
...
RAML.prototype.convertRefAttFromObject = function(object) {
  for (var id in object) {
    if (object.hasOwnProperty(id)) {
      var val = object[id];
      if (id == 'type' && typeof val === 'string' && ramlHelper.getScalarTypes.indexOf(val) < 0) {
        object.ref = val;
        delete object[id];
...
function mapMimeTypes(body, skip) {
  var result = [];
...
RAML.prototype._mapEndpoint = function(resource, baseURI, pathParams) {
  ...
  for (var i in methods) {
    ...
    if (method.body()) {
      var c = mapMimeTypes(method.body, this.data.mediaType);
      endpoint.Consumes = c.length ? c:null;
    ...
    if (method.responses()) {
      ...
      for(var code in method.responses) {
        ...
        produces = produces.concat(mapMimeTypes(method.responses[code].body, this.data.mediaType));
      }
      var p = _.uniq(produces);
      endpoint.Produces = p.length ? p:null;
  ...
  if(resources && resources.length > 0) {
    for (var i = 0; i < resources.length; i++) {
      this._mapEndpoint(resources[i], baseURI + resource.relativeUri().value(), pathParams);
    }
  }
};
...
RAML.prototype.loadData = function (data) {
  ...
  return new Promise(function(resolve, reject){
    var parsedData = parser.parseRAMLSync(data);
...
RAML.prototype._import = function() {
  ...
  this.project.Environment.DefaultResponseType = mediaType ? (Array.isArray(mediaType) ? mediaType[0] : mediaType).value() : '';
  ...
  if (this.data.mediaType !== '') {
    this.project.Environment.Produces = [this.data.mediaType];
    this.project.Environment.Consumes = [this.data.mediaType];
  }
  ...
  if (documentation) {
    for (var i in documentation) {
      var txt = new Text(documentation[i].title());
      txt.Public = true;
      txt.Content = documentation[i].content().value();
