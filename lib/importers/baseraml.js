...
RAML.prototype._mapQueryString = function(queryParameters) {
  ...
  for (var key in queryParameters) {
    ...
    queryString.properties[qp.name()] = ramlHelper.setParameterFields(qp, {});
    if (qp.required()) {
      queryString.required.push(qp.name());
    }
  }
  return queryString;
};
  ...
  for (var i in methods) {
    ...
    if (isMethod) {
      if (isMethod instanceof Array) {
				for (var i in isMethod) {
					endpoint.traits.push(isMethod[i].name());
				}
